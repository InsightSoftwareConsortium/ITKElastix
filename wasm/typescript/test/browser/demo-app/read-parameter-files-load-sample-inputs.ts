export default async function readParameterFilesLoadSampleInputs(
  model,
  preRun = false
) {
  const dataUrl =
  ...
  const parameterFilesButton = document.querySelector(
    "#readParameterFilesInputs [name=parameter-files-file-button]"
  );
  if (!preRun) {
    parameterFilesButton.loading = true;
  }
  const translationFileName = "parameters_Translation.txt";
  const translationFileResponse = await fetch(
    `${dataUrl}/${translationFileName}`
  );
  ...
  const parameterFiles = [
    { path: translationFileName, data: translationFileContent },
    { path: affineFileName, data: affineFileContent },
  ];
  model.options.set("parameterFiles", parameterFiles);

  const parameterFilesElement = document.getElementById(
    "readParameterFiles-parameter-files-details"
  );
  if (!preRun) {
