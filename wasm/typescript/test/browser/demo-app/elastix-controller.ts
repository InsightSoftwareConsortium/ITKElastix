...
import elastixLoadSampleInputs, {
  usePreRun,
  elastixLoadSample3dInputs,
} from "./elastix-load-sample-inputs.js";
...
class ElastixController {
  constructor(loadSampleInputs) {
    ...
    if (loadSampleInputs) {
      const loadSampleInputsButton = document.querySelector(
        "#elastixInputs [name=loadSampleInputs]"
      );
    ...
    const loadSample3dInputsButton = document.querySelector(
      "#elastixInputs [name=loadSample3dInputs]"
    );
    ...
    const fixedElement = document.querySelector(
      "#elastixInputs input[name=fixed-file]"
    );
    fixedElement.addEventListener("change", async (event) => {
      ...
      details.innerHTML = `<pre>${globalThis.escapeHtml(
        JSON.stringify(image, globalThis.interfaceTypeJsonReplacer, 2)
      )}</pre>`;
      details.disabled = false;
    });

    const movingElement = document.querySelector(
      "#elastixInputs input[name=moving-file]"
    );
    ...
    const resultOutputDownload = document.querySelector(
      "#elastixOutputs sl-button[name=result-download]"
    );
    ...
    resultOutputDownload.addEventListener("click", async (event) => {
      ...
      if (model.outputs.has("result")) {
        const resultDownloadFormat = document.getElementById(
          "result-output-format"
        );
        const downloadFormat = resultDownloadFormat.value || "nrrd";
        const fileName = `result.${downloadFormat}`;
        const { webWorker, arrayBuffer } = await writeImage(
          model.outputs.get("result"),
          fileName
        );
    ...
    const onSelectTab = async (event) => {
      if (event.detail.name === "elastix-panel") {
        ...
        if (
          ...
          params.get("functionName") !== "elastix"
        ) {
    ...
    const runButton = document.querySelector(
      '#elastixInputs sl-button[name="run"]'
    );
    runButton.addEventListener("click", async (event) => {
      ...
      try {
        ...
        const { result, transform, transformParameterObject } = await this.run(
          false
        );
        const t1 = performance.now();
        globalThis.notify(
          "elastix successfully completed",
        ...
        resultDetails.innerHTML = `<pre>${globalThis.escapeHtml(
          JSON.stringify(result, globalThis.interfaceTypeJsonReplacer, 2)
        )}</pre>`;
  ...
  async run(preRun) {
    let viewer = null;
    const toMultiscaleSpatialImage =
      globalThis.itkVtkViewer.utils.toMultiscaleSpatialImage;
    ...
    const { webWorker: defaultWorker, parameterMap: rigidMap } =
      await elastix.defaultParameterMap("rigid", {
    ...
    if (!preRun) {
      const { parameterMap: affineMap } = await elastix.defaultParameterMap(
        "affine",
        {
          numberOfResolutions: 3,
          webWorker: this.webWorker,
      ...
      const { parameterMap: roughSplineMap } =
        await elastix.defaultParameterMap("bspline", {
          numberOfResolutions: 3,
          finalGridSpacing: maxLength / 5,
          webWorker: this.webWorker,
        });
      parameterObject.push(roughSplineMap);
      stages.push("Rough BSpline");

      const { parameterMap: fineSplineMap } = await elastix.defaultParameterMap(
        "bspline",
        {
          numberOfResolutions: 4,
          finalGridSpacing: maxLength / 12,
          webWorker: this.webWorker,
        }
      );
      ...
      setTimeout(() => {
        const animateLabel = document.querySelector(
          'label[itk-vtk-tooltip-content="animate"]'
        );
    ...
    return {
      ...
      transformParameterObject: previousTransform,
    };
  }
}
